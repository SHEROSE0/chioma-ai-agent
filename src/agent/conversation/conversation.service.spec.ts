...
describe('ConversationService', () => {
  ...
  function makeToolRegistry(executeResult = 'tool output') {
    const execute = jest.fn<Promise<string>, Parameters<ToolRegistry['execute']>>();
    execute.mockResolvedValue(executeResult);
    const getDefinitions = jest.fn<ReturnType<ToolRegistry['getDefinitions']>, []>();
  ...
  function makeLlmProvider() {
    const complete = jest.fn<Promise<LlmCompletionResult>, [LlmCompletionRequest]>();
  ...
  it('returns the assistant reply directly when no tool call is made', async () => {
    ...
    const reply = await service.handleTurn('session-1', 'hi', { accessToken: 'tok' });
  ...
  it('executes tool calls and feeds results back into the next LLM turn', async () => {
    ...
    complete
      .mockResolvedValueOnce({
        message: {
          role: 'assistant',
          content: '',
          toolCalls: [{ id: 'call-1', name: 'get_thing', arguments: { id: '42' } }],
        },
    ...
    expect(execute).toHaveBeenCalledWith('get_thing', { id: '42' }, { accessToken: 'tok' });
  ...
  it('clears session history on resetSession', async () => {
    await sessionStore.appendMessages('session-1', [{ role: 'user', content: 'hi' }]);
